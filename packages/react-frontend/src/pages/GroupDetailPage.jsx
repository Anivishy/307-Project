...
function activitySummary(entry) {
  if (entry.eventType === 'bundleSelection') {
    return `Selected ${entry.payload?.bundleTitle ?? entry.payload?.bundleId ?? 'a bundle'}`;
  }

  if (entry.eventType === 'adminPantryEdit') {
    return 'Updated a pantry item';
  }

  if (entry.eventType === 'softPreferenceOverride') {
    return `Overrode ${entry.payload?.preference ?? 'a soft preference'}`;
  }
...
export function GroupDetailPage() {
  ...
  const [isSettingsLoading, setIsSettingsLoading] =
    useState(false);
  const [isSettingsSaving, setIsSettingsSaving] =
    useState(false);
  ...
  useEffect(() => {
    ...
    async function loadGroupDetails() {
      ...
      setErrorMessage('');
      try {
        const [groupInfoPayload, membersPayload] =
          await Promise.all([
            getGroup(groupId),
  ...
  useEffect(() => {
    ...
    async function loadActivity() {
      ...
      try {
        const payload = await getGroupActivity(groupId);
        if (isCancelled) return;
  ...
  const canEditSettings =
    isAdmin &&
    (!settings?.viewerRole || isAdminRole(settings.viewerRole));
  ...
  return (
    <section className="screen group-detail-screen">
      <div className="gd-header">
        <div>
          <p className="eyebrow">
            {isAdmin ? 'Admin view' : 'Group details'}
          </p>
      ...
      {groupInfo?.inviteCode && (
        <div
          className="invite-code-row"
          style={{ marginBottom: '1rem' }}>
          <code className="invite-code">
            {groupInfo.inviteCode}
          </code>
          <CopyButton
            text={groupInfo.inviteCode}
            label="Copy Code"
          />
          <CopyButton
            text={buildInviteLink(groupInfo.inviteCode)}
            label="Copy Link"
          />
        </div>
      )}
      ...
      <div className="gd-tab-row">
        ...
        <button
          className={`gd-tab ${activeTab === 'activity' ? 'gd-tab--active' : ''}`}
          type="button"
          ...
          {isLoading ? (
            ...
            <div className="member-grid">
              {members.map((member) => {
                ...
                return (
                  <article
                    className="member-card surface-card"
                    ...
                    <div className="member-card__info">
                      ...
                      <span
                        className={`member-role-badge member-role-badge--${memberRole.toLowerCase()}`}>
                        {memberRole}
                      </span>
                    </div>
                    <div className="member-card__pantry">
                      <p className="member-card__pantry-heading">
                        <Package size={14} />{' '}
                        {memberIngredients.length} item
                        {memberIngredients.length !== 1
                          ? 's'
                          : ''}
                      </p>
        ...
        <section className="gd-tab-content">
          ...
          {isLoading ? (
            <StatusMessage
              ...
              message="Combining all members' ingredients…"
            />
          ) : combinedPantry.length === 0 ? (
            <p style={{ opacity: 0.6, fontSize: '0.9rem' }}>
              No pantry items yet. Ask members to add
              ingredients to My Pantry.
            </p>
          ) : (
            <div className="combined-pantry-list">
              {combinedPantry.map((ing) => (
                <div
                  className="combined-pantry-row"
                  key={`${ing.ownerName}-${ing.id}`}>
                  <div className="combined-pantry-row__name">
                    <span>{ing.name}</span>
                    <span className="combined-pantry-row__owner">
                      {ing.ownerName}
                    </span>
                    {ing.lastUpdatedBy === 'bundleSelection' &&
                      ing.lastUpdatedAt && (
                        <span className="pantry-update-label">
                          Last updated by bundle selection ·{' '}
                          {new Date(
                            ing.lastUpdatedAt
                          ).toLocaleString()}
                        </span>
                  ...
                  {ing.quantity !== null &&
                    ing.quantity !== undefined && (
                      <span className="combined-pantry-row__qty">
                        {ing.quantity} {ing.unit}
                      </span>
                    )}
                </div>
        ...
        <section className="gd-tab-content">
          ...
          {isActivityLoading ? (
            <StatusMessage
              ...
              message="Fetching recent group events…"
            />
          ) : activity.length === 0 ? (
            <p style={{ opacity: 0.6, fontSize: '0.9rem' }}>
              No activity yet.
            ...
            <div className="activity-feed">
              {activity.map((entry) => (
                <article
                  ...
                  <div>
                    <strong>{activitySummary(entry)}</strong>
                    <p>
                      {entry.actorName} ·{' '}
                      {new Date(
                        entry.createdAt
              ...
              <section className="settings-card surface-card">
                ...
                <div className="toggle-row">
                  <div>
                    ...
                    <p>
                      Enable bundles that include clearly
                      disclosed shopping gaps. Disabled means
                      missing ingredients block a candidate
                      before it appears.
                    </p>
                  </div>
                  ...
                  <label
                    ...
                    <input
                      ...
                      disabled={
                        !canEditSettings || isSettingsSaving
                      }
              ...
              <section className="settings-card surface-card">
                ...
                <div className="toggle-row">
                  <div>
                    ...
                    <p>
                      When enabled, approved basics are treated
                      as effectively unlimited during group
                      recipe generation.
                    </p>
                  </div>
                ...
                <div className="staples-panel">
                  <div>
                    ...
                    <div
                      ...
                      {settings.defaultStaplesPreset.map(
                        (item) => (
                          <span
                  ...
                  <div>
                    ...
                    <p className="settings-note">
                      Add extra ingredients the household agrees
                      to treat like staples.
                    </p>
                    <div className="staple-editor">
                      ...
                      <datalist id="staple-ingredient-suggestions">
                        {settings.ingredientCatalog.map(
                          (item) => (
                            <option
                              key={item.id}
                              value={item.name}
