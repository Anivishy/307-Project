...
import { PATCH as patchSettings } from '../app/api/groups/[groupId]/settings/route';
import {
...
describe('US13 email recipient, template, and trigger behavior', () => {
  ...
  it('sends bundle-selection email to all group members with required content', async () => {
    const payload = await readCandidateSet();
    const bundleId = payload.candidates[0].id;

    const response = await selectBundle(bundleId, payload);
  ...
  it('sends pantry-update email to affected pantry owners', async () => {
    await patchSettings(
      createRequest(
    ...
    const payload = await readCandidateSet();
    const saffronBundle = payload.candidates.find(
      (candidate) =>
        candidate.id === 'bundle-saffron-pasta-night'
    );

    expect(saffronBundle).toBeDefined();
    ...
    ).toEqual(['ani@example.com', 'vinayak@example.com']);
