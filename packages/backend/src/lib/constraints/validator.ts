...
function ingredientTerms(ingredient: CandidateIngredient): Set<string> {
  const terms = [ingredient.id, ingredient.name, ...(ingredient.tags ?? [])].flatMap(matchVariants);
  return new Set(terms.filter(Boolean));
}

function ingredientMatchesText(ingredient: CandidateIngredient, constraint: string): boolean {
  ...
  return (
    ...
    Array.from(terms).some((term) => term.includes(normalizedConstraint))
  );
}
...
function collectTextViolations(
  ...
  return constraints
    .filter((constraint) => ingredientMatchesText(ingredient, constraint))
    .map((constraint) => makeViolation(userId, constraintType, constraint, ingredient, course));
}
  ...
  for (const course of candidate.courses) {
    for (const ingredient of course.ingredients) {
      for (const constraints of userConstraints) {
        ...
        if (constraints.neverIncludeIngredientIds.includes(normalizeMatch(ingredient.id))) {
          violations.push(
            makeViolation(
              constraints.userId,
              "neverInclude",
...
export function filterCandidatesByHardConstraints(
  ...
  userConstraints: UserConstraints[],
): { accepted: CandidateBundle[]; rejected: RejectedCandidate[] } {
  const accepted: CandidateBundle[] = [];
  const rejected: RejectedCandidate[] = [];

  for (const candidate of candidates) {
    const result = validateHardConstraints(candidate, userConstraints);

    if (result.allowed) {
      accepted.push(candidate);
    } else {
      rejected.push({ candidate, violations: result.violations });
    }
  }
