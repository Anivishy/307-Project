...
function isCandidateBundle(
  value: unknown
): value is CandidateBundle {
  ...
  return value.courses.every((course) => {
    if (
      ...
      !Array.isArray(course.ingredients)
    ) {
      return false;
    }
...
export async function POST(request: NextRequest) {
  ...
  try {
    currentUserId = await getCurrentUserId(request);
  } catch (error) {
  ...
  if (!currentUserId) {
    return errorResponse(
      401,
      'unauthenticated',
      'Generation requires an authenticated user.'
    );
  }
