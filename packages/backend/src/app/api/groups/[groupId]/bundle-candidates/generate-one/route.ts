...
import { generateAdditionalBundleCandidate } from "@/lib/group-service";
...
async function getDemoOrAuthenticatedUserId(request: Request) {
  ...
  // Demo-only fallback: keeps the generate-one prototype usable without a session.
...
export async function POST(
  ...
  try {
    ...
    const payload = generateAdditionalBundleCandidate(
      groupId,
